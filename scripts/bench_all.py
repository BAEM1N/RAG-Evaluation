...
def _insert_chroma(chunks, embeddings):
    ...
    client = chromadb.HttpClient(host=GATEWAY_HOST, port=8100)
    try:
...
def _insert_milvus(chunks, embeddings, dim):
    ...
    connections.connect("default", host=GATEWAY_HOST, port="19530")
...
def _insert_qdrant(chunks, embeddings, dim):
    ...
    client = QdrantClient(host=GATEWAY_HOST, port=6340, timeout=120)
    collection_name = "bench_chunks"
...
def _insert_weaviate(chunks, embeddings, dim):
    ...
    client = weaviate.connect_to_custom(
        http_host=GATEWAY_HOST, http_port=8101, http_secure=False,
        grpc_host=GATEWAY_HOST, grpc_port=50052, grpc_secure=False,
    )
...
def swap_embedding_model(model_name: str):
    ...
    with httpx.Client(timeout=120) as client:
        # 현재 실행 중인 임베딩 모델 확인
        resp = client.get(f"http://localhost:8080/v1/models")
        data = resp.json()
        ...
        for alias in running_embeds:
            client.post("http://localhost:8080/v1/models/unload", json={"model": alias})
            logger.info(f"  언로드: {alias}")

        # 새 모델 로드
        resp = client.post("http://localhost:8080/v1/models/load", json={"model": model_name})
        ...
        for _ in range(30):
            try:
                r = client.get(f"http://localhost:8080/v1/models")
