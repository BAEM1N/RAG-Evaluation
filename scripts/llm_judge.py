...
IN_DIR = Path(os.environ.get('IN_DIR', str(BASE / 'results' / 'phase5_exp_b_llm')))
OUT_DIR = Path(os.environ.get('OUT_DIR', str(BASE / 'results' / 'phase5_judge')))
...
def call_judge_once(metric: str, question: str, target: str, generated: str) -> int:
    ...
    def last_score(s: str):
        ...
        if m:
            return int(m[-1])
        # Fallback: any ASCII digit 1-5 anywhere (unicode 원형숫자 ③ 등은 isdigit=True 지만 int() ValueError → ASCII 만)
        for c in reversed(s):
            if c in '12345':
                return int(c)
        return 0
...
def _gt_lookup(qid):
    global _GT_BY_QID
    if _GT_BY_QID is None:
        gt = json.load(open(BASE / 'data' / 'ground_truth_filtered.json'))
        _GT_BY_QID = {f"q{i:03d}": item for i, item in enumerate(gt)}
    return _GT_BY_QID.get(qid, {})
...
def judge_item(idx: int, result_item: dict) -> tuple:
    ...
    # question/target 보강: 없으면 GT에서 qid로 조회
    q = result_item.get('question') or ''
    tgt = result_item.get('target_answer') or ''
    if not q or not tgt:
        qid = result_item.get('qid') or f"q{idx:03d}"
        g = _gt_lookup(qid)
        q = q or g.get('question', '')
        tgt = tgt or g.get('target_answer', '')
    votes = {}
    t0 = time.time()
...
def main():
    ...
    # .jsonl 입력 지원 (Q2 API cand 일부)
    is_jsonl = str(in_path).endswith('.jsonl')
    out_name = sys.argv[1].replace('.jsonl', '.json') if is_jsonl else sys.argv[1]
    out_path = OUT_DIR / f'judge_{judge_tag}__{out_name}'

    if is_jsonl:
        # jsonl: 각 줄 = {qid, model, question, answer, ...} → results 형식으로 변환
        rows = [json.loads(line) for line in open(in_path) if line.strip()]
        # qid 정렬 (q000, q001, ...)
        import re as _re
        def _qidx(r, idx):
            qm = _re.search(r'q(\d{3})', str(r.get('qid','')))
            return int(qm.group(1)) if qm else idx
        rows.sort(key=lambda r: _qidx(r, 0))
        # gt에서 question/target_answer 가져옴
        gt = json.load(open(BASE / 'data' / 'ground_truth_filtered.json'))
        gt_by_qid = {f"q{i:03d}": item for i, item in enumerate(gt)}
        results = []
        for r in rows:
            qm = _re.search(r'q(\d{3})', str(r.get('qid','')))
            qid = qm.group(0) if qm else None
            g = gt_by_qid.get(qid, {})
            results.append({
                'qid': qid, 'question': r.get('question') or g.get('question',''),
                'target_answer': g.get('target_answer',''),
                'generated_answer': r.get('answer') or r.get('generated_answer') or '',
            })
        data = {'llm': rows[0].get('model', 'unknown') if rows else 'unknown'}
    else:
