...
전 로컬 모델 fp32/bf16/fp16 측정 결과:

- **bf16 ≈ fp32 — 무손실** (평균차 −0.0008, 최대 |0.007|). **27B(harrier-27b)에서도 fp32 0.6177 ≈ bf16 0.618 (Δ −0.0003)** 확인. → 운영 기본은 **bf16** 권장(메모리 절반·동일 품질).
...
> 측정: `scripts/bench_phase4_parallel.py`(ST) + 공식 GGUF(llama.cpp `--embedding`). 전 곡선·26종 dtype 표는 결과 산출물 참조.
