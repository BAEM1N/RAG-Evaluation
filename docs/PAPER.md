...
| Desktop (RTX 3090 × 2way, 128GB) | (보조) |
| OpenAI AI Foundry (gpt-5.4) | Stage 4-1/5/6 LLM 변형 + 생성 + 평가 |
...
Stage 4 winner + Stage 4-2 winner 고정한 채 PreR만 변경. LLM = GPT-5.4.

| 전략 | MRR | vs baseline |
|---|---:|---:|
| query_expansion | 0.7783 | +0.0086 |
| decompose | 0.7786 | +0.0089 |
| multi_query_para | 0.7780 | +0.0083 |
| query2doc | 0.7768 | +0.0071 |
| hyde | 0.7738 | +0.0041 |
| baseline (no PreR) | 0.7697 | (기준) |
| step_back | 0.7692 | −0.0005 |

→ **Pre-retriever 단변량 효과는 사실상 noise 범위**. step_back 같은 추상화는 오히려 약간 손해. 단답형 한국어 RAG에서 query는 이미 정확하므로 변형 효과 미미.
...
| HP Z2 Mini G1a | AMD Ryzen AI Max+ 395, 128GB | Reranker (AI 395+ ROCm 7.13) |
| DGX Spark | GB10, 128GB | Phase 5 local LLM (ollama) |
| Desktop | RTX 3090 × 2-way, 128GB | 보조 |
| OpenAI AI Foundry | gpt-5.4 deployment (10M TPM / 100K RPM) | Stage 4-1/5/6 LLM 호출 |
